...
# ─── COLUMNA IZQUIERDA: PARAMETRIC COEFFICIENTS + INPUTS ───────────────
with col1:
    st.header("Parametric Coefficients")

    # Primero pedimos los inputs que necesitamos para la tabla
    ...
    st.markdown("---")
    # Ahora los inputs de velocidad y trimado, también siempre visibles
    st.subheader("Ship Speeds & Trims")
...
if predict_button:
    try:
        ...
        if len(speeds) != len(tau_list):
            st.error("Enter same number of speeds and trims.")
            st.stop()
        ...
        for v_knots, tau in zip(speeds, tau_list):
            ...
            y1 = gpr_ncg.predict(scaler1.transform(X1))[0]
            X2 = np.array([[beta, C_delta, lcg_pct, tau, H13_B, y1]])
            y2 = etr_nbow.predict(scaler2.transform(X2))[0]

            results.append({
                "Speed [knots]":      round(v_knots, 2),
                "Trim [deg]":         round(tau, 2),
                "Froude Number (Fn)": round(Fn, 3),
                "Pred nCG [g]":       round(y1, 3),
                "Pred nBow [g]":      round(y2, 3),
            })
        ...
        with col2:
            ...
            ncg_np    = df_res["Pred nCG [g]"].values
            nbow_np   = df_res["Pred nBow [g]"].values
        ...
        st.error("Only numeric values separated by commas, please.")
